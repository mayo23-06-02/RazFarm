...
const ITEMS = [
  { href: "/mill", label: "Season dashboard" },
  { href: "/mill/deliveries", label: "Deliveries" },
  { href: "/mill/quality", label: "Sucrose trends" },
];
...
export function MillNav() {
  ...
  return (
    <div className="flex items-center gap-5 border-b border-paper-200" role="tablist">
      {ITEMS.map((item) => {
        const active = pathname === item.href || (item.href !== "/mill" && pathname.startsWith(`${item.href}/`));
        return (
          <Link
            key={item.href}
            href={item.href}
